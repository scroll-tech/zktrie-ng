...
impl HashScheme for PoseidonHashScheme {
    type Error = PoseidonHashError;

    fn new_hash_try_from_bytes(bytes: &[u8]) -> Result<ZkHash, Self::Error> {
        if bytes.len() > HASH_SIZE {
            Err(PoseidonHashError::InvalidByteLength)
        } else {
            ...
            if Fr::from_repr_vartime(h).is_none() {
                return Err(PoseidonHashError::InvalidFieldElement);
            }
            Ok(ZkHash::from(h))
        }
    }
    ...
    fn raw_hash(
        ...
        let a = Fr::from_repr_vartime(le_bytes[0]).ok_or(PoseidonHashError::InvalidFieldElement)?;
        let b = Fr::from_repr_vartime(le_bytes[1]).ok_or(PoseidonHashError::InvalidFieldElement)?;
    ...
    fn hash_bytes(v: &[u8]) -> Result<ZkHash, Self::Error> {
        if v.len() >= HASH_SIZE {
            return Err(PoseidonHashError::InvalidByteLength);
        }
