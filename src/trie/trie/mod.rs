use crate::db::{HashMapDb, KVDatabase, KeyCacheDb, KeyCacheError};
use crate::hash::{poseidon::Poseidon, HashScheme, ZkHash, HASH_SIZE};
use crate::trie::{LazyNodeHash, Node, NodeType, ParseNodeError};
use crate::HashMap;
use std::fmt::Display;
    ...
    ZkTrie<MAX_LEVEL, H, Db, CacheDb>
{
    ...
    pub fn get_node(&self, node_hash: impl Into<LazyNodeHash>) -> Result<Node<H>, H, Db, CacheDb> {
        ...
        match node_hash {
            ...
            LazyNodeHash::LazyBranch { index, .. } => self
                .dirty_branch_nodes
                .get(index)
                .cloned()
                .ok_or(ZkTrieError::NodeNotFound),
    ...
    fn add_leaf(
        ...
        match n.node_type() {
            ...
            _ => {
                ...
                let lazy_hash = LazyNodeHash::LazyBranch {
